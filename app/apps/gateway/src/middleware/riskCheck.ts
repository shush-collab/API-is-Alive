...
export const riskCheck: RequestHandler = async (req, _res, next) => {
  try {
    const cooldownUntil = await redis.getNumber(req.gateway.cooldownKey);
    ...
    if (req.gateway.userAgent) {
      const userAgentCount = await redis.addToSet(userAgentKey, req.gateway.userAgent, 5 * 60 * 1000);
      if (userAgentCount > 3) {
        req.gateway.reasons.push("user_agent_changed_gt_3");
      }
    }
